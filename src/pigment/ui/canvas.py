...
class PigmentCanvas(Gtk.DrawingArea):
    def __init__(self):
        ...
        self._offset_x = 0.0     # canvas origin offset in widget coords
        self._offset_y = 0.0
    ...
    def mark_dirty(self):
        """Call this after any pixel change to trigger a redraw."""
        self._doc_dirty = True
        self.queue_draw()
    ...
    def _draw(self, area, cr: cairo.Context, width: int, height: int):
        ...
        # 2. Checkerboard (transparency pattern)
        self._draw_checker(cr)

        # 3. Document pixels
        self._draw_document(cr)

        # 4. Thin border around document
    ...
    def _draw_checker(self, cr: cairo.Context):
        ...
        # Rebuild checker pattern only when zoom changes
    ...
    def _make_checker_pattern(self) -> cairo.ImageSurface:
        # Create a 2×2 tile checkerboard as a small Cairo surface
        tile = CHECKER_SIZE
    ...
    def _draw_document(self, cr: cairo.Context):
        if not self.document:
            return
        # Rebuild Cairo surface from NumPy buffer if dirty
    ...
    def _numpy_to_cairo(self, pixels: np.ndarray) -> cairo.ImageSurface:
        ...
        h, w = pixels.shape[:2]
        # Cairo wants ARGB32 (B, G, R, A on little-endian)
        bgra = pixels[:, :, [2, 1, 0, 3]].copy()
    ...
    def _step_zoom(self, direction: int):
        """Step through the predefined zoom levels list."""
        current = self._zoom
        if direction > 0:
    ...
    def _set_zoom(self, new_zoom: float, center=False,
        ...
        """Apply a new zoom level, optionally anchoring to a widget point."""
        ...
        self._checker_zoom = -1.0  # invalidate checker cache
        if self.on_zoom_changed:
            self.on_zoom_changed(self._zoom)
        self.queue_draw()

    def _fit_to_window(self):
        """Fit the whole document into the visible area with padding."""
        if not self.document:
            return
        GLib.idle_add(self._fit_delayed)

    def _fit_delayed(self):
        """Delayed fit — runs after GTK has computed widget dimensions."""
        if not self.document:
            return
        w = self.get_width()
        h = self.get_height()
        if w <= 0 or h <= 0:
            # Widget not realized yet — try again next frame
            GLib.idle_add(self._fit_delayed)
            return
        pad = 40
        zoom_x = (w - pad * 2) / self.document.width
        zoom_y = (h - pad * 2) / self.document.height
        zoom = min(zoom_x, zoom_y, 1.0)  # never zoom in beyond 100% on fit
    ...
    def _on_scroll(self, controller, dx, dy):
        ...
        # Use widget allocation centre as fallback anchor
        wx = self.get_width()  / 2
